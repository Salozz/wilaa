...
NAME = "Wilaa"
SUDO_SLEEP_TIME = 7200 # refresh every 2 hour
...
def user_requested_full_scan(text: str) -> bool:
    ...
    keyword_match = any(phrase in text.lower() for phrase in phrases)

    if keyword_match:
        return True

    return is_user_requesting_full_scan_llm(text)

def is_user_requesting_full_scan_llm(text: str) -> bool:
    """Use LLM to determine if the user is requesting a full/comprehensive scan."""

    text_lower = text.strip().lower()

    tool_keywords = {
        "nmap", "nikto", "dirsearch",
        "run_nmap_scan", "run_nikto_scan", "run_dirsearch_scan"
    }

    # Count how many distinct tools are explicitly mentioned
    mentioned_tools = {tool for tool in tool_keywords if tool in text_lower}

    if 1 <= len(mentioned_tools) <= 2:
        return False  # User requested specific tool(s), not a full scan
    
    try:
        response = llm_internal.invoke([
            HumanMessage(content=(
                f"The user said: \"{text.strip()}\"\n"
                "Does this mean the user is asking for a full/comprehensive scan using all tools?\n"
        ...
        say(f"[ERROR] LLM full scan check failed: {e}")
        return False
...
def is_user_declining_llm(text: str) -> bool:
    """Use the internal LLM to determine if user is explicitly declining a pending action."""
    try:
        response = llm_internal.invoke([
            HumanMessage(content=(
                "You're helping detect user refusals.\n"
                f"The user said: \"{text.strip()}\"\n"
                "Does this mean the user is explicitly declining or rejecting a previously suggested action (e.g., 'no', 'don't', 'not now')?\n"
                "Reply ONLY with 'yes' or 'no'."
            ))
        ])
        ...
        say(f"[ERROR] LLM refusal check failed: {e}")
        return False
...
def is_user_requesting_reset_llm(text: str) -> bool:
    ...
    try:
        response = llm_internal.invoke([
            HumanMessage(content=(
                f"The user said: \"{text.strip()}\"\n"
                "Is the user clearly and explicitly asking to reset, restart, or start over the assistant?\n"
                "ONLY answer 'yes' if the request is unambiguous.\n"
                "Reply ONLY with 'yes' or 'no'."
    ...
    try:
        response = llm_internal.invoke([
            HumanMessage(content=(
                ...
                Your task is to determine if the user is explicitly asking to view or read the results of a previous scan (a scan report).\n

                - Reply "yes" only if the user is clearly asking to read, view, see, or access the output of a scan.\n
                - Reply "no" if the user is asking to:\n
                    - Run or start a scan (e.g., scan now, check this site, find vulnerabilities)\n
                    - Learn what the system can do (capabilities, help)\n
                    - Talk about scans in general\n
                    - Want to know information about the target
                - The word "check" alone is NOT enough. Only reply "yes" if they want to view the results.\n
                - Do NOT assume the user wants to view a report just because they mention a domain or vulnerabilities.\n
...
def main():
    ...
    while True:
        ...
        if pending_action:
            if (is_user_confirming(user_input) or is_user_confirming_llm(user_input)):
                ...
                continue
            elif is_user_declining_llm(user_input):
                pending_action = None
                say("Alright, no scan will be run then.")
                continue
            else:
                say("Hmm, irrelevant! Answer properly hooman, 'yes' or 'no'?")
                continue
        ...
        if actual_target is not None and is_target_only(user_input):
            say(f"I see a valid target ({actual_target}). Do you want me to run a full scan (Nmap, Nikto, Dirsearch)?")
       
            while True:
                confirm = user_prompt()
                if is_user_confirming(confirm) or is_user_confirming_llm(confirm):
                    run_full_scan(actual_target)
                    break
                elif is_user_declining_llm(confirm):
        ...
        nmap_pattern = r'nmap\s+((?:-[^\s]+\s*)+)\s+([a-zA-Z0-9\.-]+\.[a-zA-Z]{2,}|(?:\d{1,3}\.){3}\d{1,3})'
        nmap_matches = re.findall(nmap_pattern, str(response), flags=re.IGNORECASE)
        for flags, target in nmap_matches:
            commands_to_run.append({"tool": "run_nmap_scan", "target": target.strip(), "flags": flags.strip()})

        nikto_pattern = r'nikto\s+-h\s+([^\s]+)'
        nikto_matches = re.findall(nikto_pattern, str(response), flags=re.IGNORECASE)
        for target in nikto_matches:
            commands_to_run.append({"tool": "run_nikto_scan", "target": target.strip()})

        dirsearch_pattern = r'dirsearch\s+-u\s+([^\s]+)'
        dirsearch_matches = re.findall(dirsearch_pattern, str(response), flags=re.IGNORECASE)
        for target in dirsearch_matches:
            commands_to_run.append({"tool": "run_dirsearch_scan", "target": target.strip()})

        if commands_to_run:
            if len(commands_to_run) > 1:
                tools_text = "\n".join([f"{i+1}) {cmd['tool'].replace('run_', '').replace('_scan','')}" for i, cmd in enumerate(commands_to_run)])
                print()
                say(f"Do you want me to run the following tools on {commands_to_run[0]['target']}?\n\n{tools_text}")
                ...
                while True:
                    ...
                    if is_user_confirming(confirm) or is_user_confirming_llm(confirm):
                        ...
                        say("Alright, no scan will be run.")
                        commands_to_run = []
                        break
                    else:
                        say("Just say 'yes' or 'no', hooman.")
            if not commands_to_run:
                continue  # back to user prompt if nothing to do

            # Run the first confirmed command
            pending_action = commands_to_run.pop(0)
            extra_commands_queue = commands_to_run
            continue
        ...
        if isinstance(response, AIMessage) and hasattr(response, 'content'):
            ...
            if chosen_tools and all(tool in TOOLS for tool in chosen_tools):
                ...
                if len(chosen_tools) > 1:
                    tools_list = "\n".join(
                        [f"{i+1}) {t.replace('run_', '').replace('_scan', '')}" for i, t in enumerate(chosen_tools)]
                    )
                    print()
                    say(f"Do you want me to run the following tools on {target}?\n\n{tools_list}")
                ...
                for tool_name in chosen_tools:
                    ...
                    print()
                    say(f"Processing {tool_name.replace('_', ' ')} on {target}...")
                    print()
                    spinner = Spinner(random.choice(spinner_text))
                    spinner.start()
                    ...
                    try:
                        ...
                        spinner.stop()
                        say("Scan interrupted! But I’m still with you ≽^⊙ ˕ ◡^≼ ₊✧")
                        continue
